    ...
    public static class Jwt_Helpers
    {
        public static IEnumerable<Claim> GetClaimsByRefreshToken(this Jwt_Claims user)
        {
            return new List<Claim>()
            {
                ...
                new Claim(ClaimTypes.Actor, user.FirstName.ToString()),
        ...
        public static IEnumerable<Claim> GetClaimsByAccessToken(this Jwt_Claims user)
        {
            List<Claim> Claims = new()
            {
                ...
                new Claim(FSHClaims.BranchId, user.BranchId.ToString()),
                new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()),
                new Claim(ClaimTypes.Expiration, DateTimeOffset.UtcNow.AddDays(30).ToUnixTimeSeconds().ToString())
            };
        ...
        public static Jwt_Tokens GetAccessTokens(Jwt_Tokens jwtUser, Jwt_Claims user, Jwt_AccessTokenSettings accessTokenSettings)
        {
            ...
            jwtUser.ExpiredTime = new DateTimeOffset(DateTime.UtcNow.AddDays(30)).DateTime;
            ...
            foreach (var claim in jwtToken.Claims)
            {
                switch (claim.Type)
                {
                    case ClaimTypes.NameIdentifier:
                        claims.UserId = int.Parse(claim.Value);
                        break;
                    ...
                    case ClaimTypes.Role:
                        claims.RoleName = claim.Value;
                        break;
                    case FSHClaims.BranchId:
                        claims.BranchId = int.Parse(claim.Value);
                        break;
                }
            }
            return claims;
